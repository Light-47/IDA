...
from .betti_losses import FastBettiMatchingLoss
    ...
    def _compute_tensor(self, y_pred: torch.Tensor, y: torch.Tensor):  # type: ignore
        ...
        for i in range(y_pred.shape[0]):
            for c in range(skip_index, y_pred.shape[1]):
                ...
                results = betti_matching.compute_matching(predictions_cpu_batch, targets_cpu_batch, include_input2_unmatched_pairs=True)
                dim_0, dim_1 = (0, 1) if not self.eight_connectivity else (1, 0)

                # Compute betti number error
                # 计算预测的Betti数
                b0_pred = results[0].num_matched[dim_0] + results[0].num_unmatched_input1[dim_0] + 1
                b1_pred = results[0].num_matched[dim_1] + results[0].num_unmatched_input1[dim_1]

                # 计算标签的Betti数
                b0_label = results[0].num_matched[dim_0] + results[0].num_unmatched_input2[dim_0] + 1
                b1_label = results[0].num_matched[dim_1] + results[0].num_unmatched_input2[dim_1]

                b0_errors[i, c - skip_index] = abs(b0_pred - b0_label)
                b1_errors[i, c - skip_index] = abs(b1_pred - b1_label)

                # Compute betti matching error
                bm_losses[i, c - skip_index] = self.BM_loss._betti_matching_loss(y_pred[i, c], y[i, c], results[0])
