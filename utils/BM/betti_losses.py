...
import sys
sys.path.append('/home/xl/thinclient_drives/spw/AAATrial/SkelCon-main/Betti-Matching-3D/build')
import betti_matching           # C++ Implementation

import typing
# from .dice_losses import Multiclass_CLDice
from .utils import FiltrationType, ActivationType
...
class FastBettiMatchingLoss(_Loss):
    ...
    def _betti_matching_loss(self,
        ...
        def stack_or_empty(arrays):
                # 处理空 list 或 list 中所有元素为空
                if not arrays or all(len(a) == 0 for a in arrays):
                    return torch.zeros(0, len(prediction.shape), device=prediction.device, dtype=torch.long)
                arrays = [torch.tensor(a, device=prediction.device, dtype=torch.long) for a in arrays if len(a) > 0]
                return torch.cat(arrays, dim=0)  # 拼接所有数组（N, D）

        # 拼接所有匹配 / 不匹配坐标（转为 torch）
        prediction_matches_birth_coordinates = stack_or_empty(betti_matching_result.input1_matched_birth_coordinates)
        prediction_matches_death_coordinates = stack_or_empty(betti_matching_result.input1_matched_death_coordinates)
        target_matches_birth_coordinates = stack_or_empty(betti_matching_result.input2_matched_birth_coordinates)
        target_matches_death_coordinates = stack_or_empty(betti_matching_result.input2_matched_death_coordinates)

        prediction_unmatched_birth_coordinates = stack_or_empty(betti_matching_result.input1_unmatched_birth_coordinates)
        prediction_unmatched_death_coordinates = stack_or_empty(betti_matching_result.input1_unmatched_death_coordinates)
        target_unmatched_birth_coordinates = stack_or_empty(betti_matching_result.input2_unmatched_birth_coordinates)
        target_unmatched_death_coordinates = stack_or_empty(betti_matching_result.input2_unmatched_death_coordinates)
